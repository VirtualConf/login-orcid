...
/**
 * Class LoginOrcidInstaller
 * 
 */

class LoginOrcidInstaller extends WireData {
	...
	protected function execute($mode) {
		...
		if(!$field) {
			...
			$field->description = 'ORCID ID as populated by LoginOrcid module.';
		...
		if(!$userFieldgroup->hasField($field)) {
			// Add the ORCID ID field to the user fieldgroup
